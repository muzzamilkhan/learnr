...
for the verbatim signatures): `kind`, `build`, `issues`, and `fields` - plus a fifth,
optional `answerIssues` that Task 12 (`array`) added to the contract; see
`docs/superpowers/notes/figure-kind-author-notes.md` section 1 for when a later kind
needs it too. `fields` declares the kind's authored parameters and which are
