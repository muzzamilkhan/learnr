 ...
 * once through `buildProfile`.
 *
 * **`buildProfile` sorts by `answeredAt` before folding, and that is why both
 * are here.** The out-of-order undercount can only ever appear on the
 * `nextSkill` path, which is the path the stored `TopicSkill` row takes in
 * production - one answer folding onto the last as it arrives. Fold only
 * through `buildProfile` and `out-of-order-days` produces exactly what the
 * sorted sequence produces, pinning nothing.
 */
