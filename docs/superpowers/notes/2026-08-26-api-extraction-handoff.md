...
The template schema's real guard is not the compiler. `Mirrored` catches a field
dropped from a DTO, but a figure's fields are mostly optional - `rotation`,
`mirror`, `armLength` and the rest - and most of what a shipped template pins is
exactly one of those. A schema that quietly stripped an optional field would
still satisfy every compile-time check in `dto.ts`; what actually catches it is
`packs.test.ts` and `content.test.ts` comparing the served and generated bytes
against the committed JSON, byte for byte. That is the round-trip test's whole
job, and it is the one this note's "verify by breaking the guard" method was
run against here too.
